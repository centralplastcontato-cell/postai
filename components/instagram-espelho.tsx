    ...
    <div>
      ...
      {feed.length === 0 ? (
        ...
        <div className="grid grid-cols-3 gap-1 sm:gap-2">
          {feed.map((p) => (
            <a key={p.id} href={p.permalink || undefined} target="_blank" rel="noreferrer" title={p.legenda?.slice(0, 120)} className="group relative aspect-square overflow-hidden rounded-md bg-preto sm:rounded-lg">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={p.imagem} alt="" loading="lazy" className="h-full w-full object-cover transition group-hover:scale-105" />
