...
export function FestasPainel({ marcaId, linkBase, token: tokenInicial, festas }: {
  ...
  async function gerarNovo() {
    if (!confirm("Gerar um link NOVO desativa o atual — quem tiver o link antigo não consegue mais subir fotos. Continuar?")) return;
    await gerar();
  }

  async function revogar() {
    if (!confirm("Desativar o link? Quem tiver o link atual deixa de conseguir subir fotos.")) return;
    setErro(null);
    try {
  ...
  async function copiar() {
    try {
      ...
      setErro("Não consegui copiar. Copie manualmente o link acima.");
    }
  }

  async function apagarFesta(id: string, nome: string) {
    if (!confirm(`Excluir a festa de ${nome}? As fotos continuam no Banco de imagens — só some o agrupamento por festa.`)) return;
    await excluirFesta(id);
    router.refresh();
  }
        ...
        {token ? (
          <>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <input readOnly value={link} onClick={(e) => (e.target as HTMLInputElement).select()} className="input-base flex-1 min-w-[200px] text-xs" />
            ...
            <div className="mt-2 flex flex-wrap gap-3 text-xs">
              ...
              <button type="button" onClick={revogar} className="text-red-400 underline transition hover:text-red-300">Desativar link</button>
            </div>
          </>
        ) : (
            ...
            {festas.map((f) => (
              <div key={f.id} className="rounded-lg border border-linha bg-preto p-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-sm font-semibold text-white">🎂 {f.aniversariante}{f.tema ? <span className="font-normal text-muted"> · {f.tema}</span> : null}</p>
                    <p className="text-xs text-muted">{dataBR(f.dataISO)} · {f.fotos.length} {f.fotos.length === 1 ? "foto" : "fotos"}</p>
                  </div>
                  <button type="button" onClick={() => apagarFesta(f.id, f.aniversariante)} title="Excluir festa" className="shrink-0 rounded px-2 py-1 text-xs text-red-400 transition hover:bg-red-900/30">✕ Excluir</button>
                </div>
                {f.fotos.length > 0 && (
