...
export function MarcaHub({
  ...
  const [aba, setAba] = useState<"redes" | "imagens" | "festas" | "video" | "mascote" | "instagram" | "paginas" | "campanhas" | "config">("redes");
  const cls = (a: boolean) =>
  ...
  return (
    <div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2">
        ...
        <button onClick={() => setAba("video")} className={cls(aba === "video")}>🎬 Vídeo</button>
      ...
      <div className="mt-6">
        {aba === "redes" && <RedesSociais marcaId={marca.id} posts={posts} publicacoes={publicacoes} stories={stories} reels={reels} festasComVideo={festasComVideo} diasCarrossel={marca.diasCarrossel} diasFeed={marca.diasFeed} horaPost={marca.horaPost} horaCarrossel={marca.horaCarrossel} paleta={marca.paleta} temFacebook={Boolean(marca.fbPageId)} espelharStoryPadrao={marca.espelharStory} sugestao={sugestao} feedArtes={marca.feedArtes ?? []} temMascote={Boolean(marca.mascoteUrl)} />}
        ...
        {aba === "video" && <VideoPainel marcaId={marca.id} festas={festas} tematicos={videosTematicos} corMarca={marca.corPrimaria} capasBanco={marca.capasArte ?? []} />}
        {aba === "mascote" && <MascoteEstudio marcaId={marca.id} mascoteUrl={marca.mascoteUrl ?? ""} mascotes={marca.mascotesArte ?? []} />}
