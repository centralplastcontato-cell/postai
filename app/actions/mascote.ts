...
export async function gerarFicha3d(marcaId: string) {
  ...
  try {
    ...
    const form = new FormData();
    form.append("model", "gpt-image-1");
    form.append("prompt", "Prancha de referência (model sheet / turnaround) do MESMO personagem da imagem, para modelagem 3D. Mostre o personagem de CORPO INTEIRO em TRÊS vistas lado a lado, na mesma escala e alinhadas: 1) VISTA DE FRENTE, 2) VISTA DE LADO (perfil), 3) VISTA DE COSTAS. MANTENHA TODOS os elementos do personagem IDÊNTICOS à referência, INCLUSIVE a BANDEIRINHA no topo (mantenha a bandeira nas três vistas; o texto dela não precisa ficar legível). MUITO IMPORTANTE: deixe uma MARGEM boa em volta e mostre o personagem INTEIRO em cada vista — do TOPO (a bandeira) até os PÉS — sem CORTAR nenhuma parte (nem o topo, nem os lados, nem os pés). Pose neutra em pé (T-pose leve), proporções e cores iguais nas três vistas, mesmo estilo 3D fofo. Fundo cinza-claro liso de estúdio, iluminação uniforme, alto nível de detalhe. Sem legendas, sem números, sem molduras ao redor.");
