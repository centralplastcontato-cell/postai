...
- **Motor de vídeo (monta os Reels) NÃO está neste repo.** É um serviço Node + ffmpeg no Google Cloud Run (projeto `postai-video-engine`, serviço `video-engine`, região `southamerica-east1`), chamado por `lib/video-engine.ts` em `${VIDEO_ENGINE_URL}/montar`; ele avisa de volta em `/api/video-pronto`. Código: `C:\projetos\video-engine` no PC do Victor + repo privado https://github.com/centralplastcontato-cell/postai-video-engine. Reimplantar = `gcloud run deploy video-engine --source <pasta> --region southamerica-east1 --project postai-video-engine` (só o Victor, logado no gcloud).
